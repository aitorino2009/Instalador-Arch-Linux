...
* **Particionado Personalizado (/home separado):** Implementado en la rama `particionado_personalizado`. El script pregunta si se desea separar `/home` y cuál será el tamaño de la raíz (`/`). El espacio restante va a `/home` automáticamente. La lógica maneja los 4 escenarios posibles (con/sin swap × con/sin `/home`) tanto en UEFI (`sgdisk`) como en BIOS (`parted`), usando la función auxiliar `_a_mb()` para convertir tamaños a MB. El montaje es jerárquico: primero root, luego home, luego EFI. `genfstab -U` detecta `/mnt/home` de forma nativa.
